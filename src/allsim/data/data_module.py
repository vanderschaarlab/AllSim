...
class OrganDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        if stage == "fit" or stage is None:
            X = torch.tensor(X_train.to_numpy(), dtype=torch.double)  # noqa E741
            O = torch.tensor(O_train.to_numpy(), dtype=torch.double)  # noqa E741
            Y = torch.tensor(Y_train.to_numpy(), dtype=torch.double).view(
                -1, 1
            )  # noqa E741
        ...
        if stage == "test" or stage is None:
            X = torch.tensor(X_test.to_numpy(), dtype=torch.double)  # noqa E741
            O = torch.tensor(O_test.to_numpy(), dtype=torch.double)  # noqa E741
            Y = torch.tensor(Y_test.to_numpy(), dtype=torch.double).view(
                -1, 1
            )  # noqa E741
